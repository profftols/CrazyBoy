...
namespace YG.EditorScr
{
    ...
    public class PlatformSettingsEditor : Editor
    {
        ...
        private void OnEnable()
        {
            ...
            var modulName = Path.GetFileName(folderPath);
            ...
            var iconPath = GetIconCurrentPlatformPath(modulName);

            if (File.Exists(iconPath))
            {
                var fileData = File.ReadAllBytes(iconPath);
        ...
        {
            var iconPath = Path.Combine(InfoYG.PATCH_PC_WEBGLTEMPLATES, modulName, "thumbnail.png");
            if (File.Exists(iconPath))
                return iconPath;
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (iconPlatform)
            {
                var textureRect = GUILayoutUtility.GetRect(45, 45, GUILayout.ExpandWidth(false));
            ...
            var styleNamePlatform = TextStyles.White();
            ...
            var tooltip = new GUIContent(scr.nameFull + "_yg", Langs.t_nameDefining);
        ...
        {
            var fields = scrObject.GetType().GetFields(BindingFlags.Public | BindingFlags.Instance);
            object toggleScrObject = YG2.infoYG.platformToggles;
            var toggleFields = toggleScrObject.GetType().GetFields(BindingFlags.Public | BindingFlags.Instance);
            ...
            foreach (var field in fields)
            {
                if (checkPlatform != null)
                {
                    ...
                    if (platformAttribute == null || platformAttribute.name != checkPlatform) continue;
                }
                ...
                for (var i = 0; i < toggleFields.Length; i++)
                {
                    ...
                    if (toggle.Name == field.Name)
                    {
                        ...
                        var toggleValue = (bool)toggle.GetValue(toggleScrObject);
                        var newToggleValue = EditorGUILayout.Toggle(toggleValue, GUILayout.Width(20));
        ...
        private void DrawField(FieldInfo field, object target)
        {
            ...
            var valueWidth = GUILayout.Width(120);
