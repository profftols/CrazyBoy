...
namespace YG.EditorScr
{
    ...
    {
        public static List<Module> GetGeneratedList()
        {
            return GetGeneratedList(ServerInfo.saveInfo);
        }
        ...
        public static List<Module> GetGeneratedList(ServerJson cloud)
        {
            ...
            // PluginYG2 module
            var pluginVersionPatch = InfoYG.PATCH_PC_YG2 + "/Version.txt";
            ...
            for (var i = 0; i < modulesTextLines.Length; i++)
            {
                if (modulesTextLines[i] == string.Empty)
                    continue;

                var name = modulesTextLines[i];
                var version = "imported";

                var spaceIndex = modulesTextLines[i].IndexOf(' ');
                if (spaceIndex > -1)
                {
            ...
            // Platforms
            var platfomFolders = Directory.GetDirectories(InfoYG.PATCH_PC_PLATFORMS);
            var platfomNames = new string[platfomFolders.Length];

            for (var i = 0; i < platfomFolders.Length; i++)
                platfomNames[i] = Path.GetFileName(platfomFolders[i]);

            for (var i = 0; i < platfomNames.Length; i++)
            {
                var version = "imported";
                var platfomVersionPathc = $"{InfoYG.PATCH_PC_PLATFORMS}/{platfomNames[i]}/Version.txt";
            ...
            if (cloud != null && cloud.modules.Length > 0)
                for (var i = 0; i < cloud.modules.Length; i++)
                {
                    var found = false;
            ...
            var selectModulesList = PrefsList.GetList(VersionControlWindow.SELECT_MODULES_KEY);
