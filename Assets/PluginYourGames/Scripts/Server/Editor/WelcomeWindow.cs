...
namespace YG.EditorScr
{
    ...
    public class WelcomeWindow : EditorWindow
    {
        ...
        private static void InitializeOnLoad()
        {
            ...
            {
                if (PluginPrefs.GetInt(InfoYG.FIRST_STARTUP_KEY) == 1)
                {
        ...
        {
            var window = GetWindow<WelcomeWindow>($"Welcome to {InfoYG.NAME_PLUGIN}!");
        ...
        private void OnGUI()
        {
            var styleWelcome = TextStyles.LabelStyleColor(Color.white);
            ...
            var buttonStyle = new GUIStyle(YGEditorStyles.button);
...
#else
            var settingsButtonText = "Plugin settings";
#endif
            ...
            if (Server.loadComplete)
            {
...
#else
                GUILayout.Label("All the useful information is in the documentation and telegram channel",
                    styleWelcome);
#endif
                ...
                for (var i = 0; i < ServerInfo.saveInfo.modules.Length; i++)
                    if (ServerInfo.saveInfo.modules[i].name == InfoYG.NAME_PLUGIN)
                    {
                        cloudVersionStr = ServerInfo.saveInfo.modules[i].version;
                        float.TryParse(cloudVersionStr, NumberStyles.Float, CultureInfo.InvariantCulture,
                        ...
                        if (ServerInfo.saveInfo.modules[i].critical)
                ...
                GUILayout.Space(10);
                var textureRect = GUILayoutUtility.GetRect(50, 50, GUILayout.ExpandWidth(false));
...
#else
                    GUILayout.Label($"\nYou have the latest version of {InfoYG.NAME_PLUGIN}!  (v{InfoYG.VERSION_YG2})",
                        styleWelcome);
#endif
                }
                else
...
#else
                    GUILayout.Label($"\nThere is a more recent version of {InfoYG.NAME_PLUGIN}!  (v{cloudVersionStr})",
                        styleWelcome);
#endif
                }
