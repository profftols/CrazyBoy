    ...
    public class CwInputManager : MonoBehaviour
    {
        ...
        public struct Axis
        {
            ...
            public float GetValue(float delta)
            {
                ...
                value -= CwInput.GetKeyIsHeld(KeyNegative) ? KeySensitivity * delta : 0.0f;
                value += CwInput.GetKeyIsHeld(KeyPositive) ? KeySensitivity * delta : 0.0f;

                value -= CwInput.GetKeyIsHeld(KeyNegativeAlt) ? KeySensitivity * delta : 0.0f;
                value += CwInput.GetKeyIsHeld(KeyPositiveAlt) ? KeySensitivity * delta : 0.0f;
                ...
                if (FingerCount > 0 && fingers.Count == FingerCount)
                {
                    ...
                    switch (FingerGesture)
                    {
                        case AxisGesture.HorizontalDrag:
                            value += GetAverageDeltaScaled(fingers).x * FingerSensitivity * scale; break;
                        case AxisGesture.VerticalDrag:
                            value += GetAverageDeltaScaled(fingers).y * FingerSensitivity * scale; break;
                        case AxisGesture.Twist: value += GetAverageTwistRadians(fingers) * FingerSensitivity; break;
                        case AxisGesture.HorizontalPull:
                            value += GetAveragePullScaled(fingers).x * FingerSensitivity * delta * scale; break;
                        case AxisGesture.VerticalPull:
                            value += GetAveragePullScaled(fingers).y * FingerSensitivity * delta * scale; break;
                    }
                }
        ...
        public struct Trigger
        {
            ...
            public bool WentDown(Finger finger)
            {
                ...
                if (UseKey != KeyCode.None && finger.Index == HOVER_FINGER_INDEX &&
                    CwInput.GetKeyWentDown(UseKey)) return true;
            ...
            public bool IsDown(Finger finger)
            {
                ...
                if (UseKey != KeyCode.None && finger.Index == HOVER_FINGER_INDEX && CwInput.GetKeyIsHeld(UseKey))
                    return true;
            ...
            public bool WentUp(Finger finger, bool useAnyFinger = false)
            {
                ...
                if (UseKey != KeyCode.None && finger.Index == HOVER_FINGER_INDEX && CwInput.GetKeyWentUp(UseKey))
                    return true;
        ...
        public abstract class Link
        {
                ...
                where T : Link
            {
                if (links != null)
                {
                    foreach (var link in links) link.Clear();
        ...
        public class Finger
        {
            ...
            public float SmoothScreenPositionDelta
            {
                get
                {
                    if (!Up) return Vector2.Distance(ScreenPositionOldOld, ScreenPositionOld);
        ...
        private static readonly List<Finger> filteredFingers = new();

        private static readonly Stack<Finger> pool = new();
            ...
            if (currentEventSystem != null)
            {
                ...
                if (tempRaycastResults.Count > 0)
                    for (var i = tempRaycastResults.Count - 1; i >= 0; i--)
                    {
                        ...
                        if ((raycastLayer & guiLayers) == 0) tempRaycastResults.RemoveAt(i);
                    }
            }
        ...
        public static Vector2 GetAveragePosition(List<Finger> fingers)
        {
            ...
            foreach (var finger in fingers) total += finger.ScreenPosition;
        ...
        public static Vector2 GetAverageOldPosition(List<Finger> fingers)
        {
            ...
            foreach (var finger in fingers) total += finger.ScreenPositionOld;
        ...
        public static Vector2 GetAveragePullScaled(List<Finger> fingers)
        {
            ...
            foreach (var finger in fingers) total += finger.ScreenPosition - finger.StartScreenPosition;
        ...
        public static Vector2 GetAverageDeltaScaled(List<Finger> fingers)
        {
            ...
            foreach (var finger in fingers) total += finger.ScreenPosition - finger.ScreenPositionOld;
        ...
        public static float GetAverageTwistRadians(List<Finger> fingers)
        {
            ...
            foreach (var finger in fingers) total += GetDeltaRadians(finger, center, oldCenter);
        ...
        protected virtual void Update()
        {
            ...
            else if (CwInput.GetMouseExists())
            {
                ...
                if (mouseSet || mouseUp) AddFinger(MOUSE_FINGER_INDEX, CwInput.GetMousePosition(), 1.0f, mouseSet);
            }

            // Events
            foreach (var finger in Fingers)
            {
                if (finger.Down && OnFingerDown != null) OnFingerDown.Invoke(finger);
                if (OnFingerUpdate != null) OnFingerUpdate.Invoke(finger);
                if (finger.Up && OnFingerUp != null) OnFingerUp.Invoke(finger);
            }
        }
