...
namespace Lean.Common
{
    ...
    public class LeanSelectable : MonoBehaviour
    {
        ...
        public void Deselect()
        {
            ...
            foreach (var select in LeanSelect.Instances) select.Deselect(this);
        }

        /// <summary>This deselects all objects in the scene.</summary>
        public static void DeselectAll()
        {
            foreach (var select in LeanSelect.Instances) select.DeselectAll();

            foreach (var selectable in Instances) selectable.SelfSelected = false;
        }

        public void InvokeOnSelected(LeanSelect select)
        {
            if (onSelected != null) onSelected.Invoke(select);

            if (OnAnySelected != null) OnAnySelected.Invoke(select, this);
        }

        public void InvokeOnDeslected(LeanSelect select)
        {
            if (onDeselected != null) onDeselected.Invoke(select);

            if (OnAnyDeselected != null) OnAnyDeselected.Invoke(select, this);
        }
        ...
        protected virtual void OnEnable()
        {
            ...
            if (OnAnyEnabled != null) OnAnyEnabled.Invoke(this);
        }
        ...
        protected virtual void OnDisable()
        {
            ...
            if (OnAnyDisabled != null) OnAnyDisabled.Invoke(this);
        }
...
namespace Lean.Common.Editor
{
    ...
    public class LeanSelectable_Editor : CwEditor
    {
        ...
        {
            if (showUnusedEvents || Any(tgts, t => t.OnSelected.GetPersistentEventCount() > 0)) Draw("onSelected");

            if (showUnusedEvents || Any(tgts, t => t.OnDeselected.GetPersistentEventCount() > 0)) Draw("onDeselected");
        }
    }
}
#endif